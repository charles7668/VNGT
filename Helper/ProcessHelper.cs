...
namespace Helper
{
    ...
    {
        private const double PROCESS_STOP_WATCHER_INTERVAL = 0.5;
        private static ManagementEventWatcher? _ProcessStopWatcher;

        /// <summary>
        /// Registers a callback for process stop events.
        /// </summary>
        /// <param name="callback">The callback to register.</param>
        [UsedImplicitly]
        public static void RegisterProcessStopCallback(EventArrivedEventHandler callback)
        {
            if (!RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                return;
            if (_ProcessStopWatcher == null)
                return;
            _ProcessStopWatcher.EventArrived += callback;
        }

        /// <summary>
        /// Unregisters a callback for process stop events.
        /// </summary>
        /// <param name="callback">The callback to unregister.</param>
        [UsedImplicitly]
        public static void UnregisterProcessStopCallback(EventArrivedEventHandler callback)
        {
            if (!RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                return;
            if (_ProcessStopWatcher == null)
        ...
        /// <summary>
        /// Stops the process stop watcher and releases its resources.
        /// </summary>
        [UsedImplicitly]
        public static void StopProcessStopWatcher()
        {
            if (!RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                return;
            if (_ProcessStopWatcher == null)
        ...
        /// <summary>
        /// Starts the process stop watcher to monitor for process termination events.
        /// </summary>
        [UsedImplicitly]
        public static void StartProcessStopWatcher()
        {
            if (!RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                return;
            if (_ProcessStopWatcher != null)
            ...
            string query =
                $"SELECT * FROM __InstanceDeletionEvent WITHIN {PROCESS_STOP_WATCHER_INTERVAL} WHERE TargetInstance ISA 'Win32_Process'";

            _ProcessStopWatcher = new ManagementEventWatcher(new WqlEventQuery(query));
